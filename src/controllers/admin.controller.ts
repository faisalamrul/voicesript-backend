...
export async function getEditorDetail(
  ...
  try {
    ...
    const [profile, stats, jobsResult] = await Promise.all([
      userRepo.findPublicById(id),
      userRepo.getEditorStats(id),
      userRepo.getEditorJobs(id, { search, page, limit }),
    ]);
    if (!profile) throw AppError.notFound('Editor not found');
    if (profile.role !== 'editor') throw new AppError('User is not an editor', 400);
    sendSuccess(res, {
      profile,
      stats,
      ...
      pagination: {
        ...
        page,
        limit,
        total_pages: Math.ceil(jobsResult.total / limit),
      },
    });
  } catch (err) {
    next(err);
...
export async function getReporterDetail(
  req: AuthenticatedRequest,
  res: Response,
  next: NextFunction
): Promise<void> {
  try {
    ...
    const [profile, stats, jobsResult] = await Promise.all([
      userRepo.findPublicById(id),
      userRepo.getReporterStats(id),
      userRepo.getReporterJobs(id, { search, page, limit }),
    ]);
    if (!profile) throw AppError.notFound('Reporter not found');
    if (profile.role !== 'reporter') throw new AppError('User is not a reporter', 400);
    sendSuccess(res, {
      profile,
      stats,
      jobs: jobsResult.jobs,
