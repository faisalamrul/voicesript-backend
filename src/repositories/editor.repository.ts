...
export interface EditorEarningsFilters {
  search?: string;
  period?: 'month' | 'last';
  page?: number;
  limit?: number;
...
export interface EditorEarningsJob {
  id: string;
  case_name: string;
  location: string;
  duration: number;
...
export async function getEarnings(
  editorId: string,
  filters: EditorEarningsFilters
): Promise<EditorEarningsResult> {
  const now = new Date();
  ...
  const statsQuery = pool.query<{
    ...
    `SELECT
      ...
      COALESCE(SUM(CASE WHEN status = 'REVIEWED'  THEN ${EDITOR_FLAT_FEE} END), 0)         AS pending,
      COALESCE(SUM(CASE WHEN status = 'COMPLETED'
                        AND completed_at >= $2 AND completed_at < $3
                   THEN editor_payment END), 0)                                             AS period_earned,
      COUNT(*)                                                                               AS total_jobs,
  ...
  // Query B — turnaround stats (editor_assigned_at → reviewed_at, recent 3mo vs older baseline)
  const turnaroundQuery = pool.query<{
    avg_hours: string;
    recent_avg_hours: string;
    baseline_avg_hours: string;
    reviewed_jobs_count: string;
  }>(
    `SELECT
      COALESCE(AVG(EXTRACT(EPOCH FROM (reviewed_at - editor_assigned_at)) / 3600), 0)       AS avg_hours,
      COALESCE(AVG(CASE WHEN reviewed_at >= $2
                   THEN EXTRACT(EPOCH FROM (reviewed_at - editor_assigned_at)) / 3600 END), 0) AS recent_avg_hours,
      COALESCE(AVG(CASE WHEN reviewed_at < $2
                   THEN EXTRACT(EPOCH FROM (reviewed_at - editor_assigned_at)) / 3600 END), 0) AS baseline_avg_hours,
      COUNT(*)                                                                               AS reviewed_jobs_count
     FROM jobs
     WHERE editor_id = $1
       AND editor_assigned_at IS NOT NULL
       AND reviewed_at IS NOT NULL`,
    [editorId, threeMonthsAgo]
  );
  ...
  const chartQuery = pool.query<{ month: string; earned: string; minutes: string; jobs: string }>(
    `SELECT
      ...
      COALESCE(SUM(editor_payment), 0)                      AS earned,
     ...
     ORDER BY 1`,
    [editorId, sixMonthsAgo]
  );

  // Query D — jobs list (period-filtered, paginated)
  const jobValues: unknown[] = [editorId];
  const jobConditions: string[] = ['j.editor_id = $1'];
  ...
  if (periodRange) {
    ...
    jobConditions.push(`(
      ...
      OR
      (j.status != 'COMPLETED' AND j.editor_assigned_at >= $${startIdx} AND j.editor_assigned_at < $${endIdx})
    )`);
  }
  ...
  const jobsQuery = pool.query<{
    ...
    `SELECT
      j.id, j.case_name, j.location, j.duration, j.status, j.editor_payment,
      j.editor_assigned_at AS assigned_at,
      j.reviewed_at, j.completed_at,
      COUNT(*) OVER() AS _total
     FROM jobs j
     WHERE ${jobConditions.join(' AND ')}
     ORDER BY COALESCE(j.completed_at, j.editor_assigned_at, j.created_at) DESC
  ...
  const turnaround_stats: TurnaroundStats = {
    ...
    reviewed_jobs_count: parseInt(tRaw.reviewed_jobs_count, 10),
  ...
  const jobs: EditorEarningsJob[] = jobsRes.rows.map(({ _total: _t, ...row }) => ({
    id: row.id,
