...
export function calculateReporterPay(durationSeconds: number): number {
  return Math.floor(durationSeconds / 60) * REPORTER_PAY_RATE;
}
...
export async function getJobById(jobId: string, requestingUser: { id: string; role: string }): Promise<Job> {
  const job = await jobRepo.findByIdWithNames(jobId);
  if (!job) throw AppError.notFound('Job not found');
  if (requestingUser.role === 'reporter' && job.reporter_id !== requestingUser.id) {
